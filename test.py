...
samples_C=np.linspace(C_in, C_fin, 20)
dx2=0.3
K_fin=100
K_in=40
grid_K  = int(K_fin/dx2)  - int(K_in/dx2) +1
samples_K=np.linspace(K_in, K_fin, 20)
...
# Assuming necessary imports and definitions for model, init_cond, t, etc.
...
for M in range(20):
    f = gamma * samples_K[M]
    for i in range(20):
        evals = [odeint(model, init_cond, t, args=(samples_K[M], w, f, samples_C[i]))]
        LCE(samples_K[M], samples_C[i],i,M,ll_1,ll_2, evals[i],Qk,Rk,Qkm,Ykm,Y0,Yk,R)
