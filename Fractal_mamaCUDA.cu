...
__device__ void myqr(float* A,float* Q, float* R){
    ...
    for(int k = 0; k < m ; k++){
        for(int j = 0; j < k-1; j++){
            ...
            R[j + n*k] = sum;
            for (int i = 0; i < n; ++i) {
               A[i + n*k] = A[i + n*k] - R[j + n*k] * Q[i + n*j];
            }
        }
        float norm_squared = 0.0f;
        for (int i = 0; i < n; ++i) {
            norm_squared += A[i + n*k]*A[i + n*k];
        }
        R[k + n*k] = sqrtf(norm_squared);
        // Update Q
        for (int i = 0; i < n; ++i) {
    ...
    for(int i = 1; i < max_iter; i++)
        {
            ...
            for(int j = 0; j < 2; j++){
                for(int k = 0;k < 2; k++){
                    d_JM_plus[j + 2*k]  = d_H[j + 2*k] + 0.5 * DT * A[j + 2*k];
                    d_JM_minus[j + 2*k] = d_H[j + 2*k] - 0.5 * DT * A[j + 2*k];
                }
            }
            inverse(d_JM_minus);
            for(int j = 0; j < 2; j++){
