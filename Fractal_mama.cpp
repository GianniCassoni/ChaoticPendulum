...
struct pendulum_ode
{
    ...
    pendulum_ode( const double &K,  const double &C, const double &f) : K_value( K ),  C_value( C ), f_value(f){ }

    void operator()( const state_type &x , state_type &dxdt, double t ) const
    {
        dxdt[0] = x[1];
        dxdt[1] = f_value * cos(data.w * t) - K_value*sin(x[0]) -C_value* x[1];
    }
};
...
std::pair<double, double> LCE(const size_t &steps,const vector<state_type> &x_vec,const vector<double> &times, const double &K,  const double &C) {
    ...
    for (size_t i = 1; i <=  steps; i++){
        ...
        MatrixXd A = JM(x_vec[i][1], C, K); // Assuming A is already defined as a MatrixXd
...
int main() {
    ...
    // MatrixXd v0 = MatrixXd::Ones(2, 1); // Initial condition
    // MatrixXd Qk = MatrixXd::Identity(2, 2);
    // MatrixXd Y0 = MatrixXd::Identity(2, 2);
    // MatrixXd Yk = MatrixXd::Identity(2, 2);
    // MatrixXd Rk = MatrixXd::Identity(2, 2);
    // MatrixXd R  = MatrixXd::Identity(2, 2);ù
    ...
    if (!std::filesystem::exists("data")) {
        ...
        // If it does exist, remove all files inside it
    ...
    for (size_t i = 0; i < data.K_elem; ++i)
    {
        for (size_t j = 0; j < data.C_elem; ++j)
        {
            ...
            //// Construct file name based on i and j
            //stringstream filename_stream;
            //filename_stream << "data/data_" << i << "_" << j << ".csv";
            //string filename = filename_stream.str();

            state_type x_0 = { .0 , .0}; // initial conditions
            double f = data.gamma * data.samples_K[i];
            size_t steps = integrate_const(stepper, pendulum_ode( data.samples_K[i], data.samples_C[j], f),  x_0 , 0.0 , data.t_max , data.dt, push_back_state_and_time(x_vec , times));
            ...
            //ofstream outfile(filename); // Open a file for writing  
            //// Output data to file
            //for (size_t k = 0; k <= steps; k++) {
            //    outfile << times[k] << ',' << x_vec[k][0] << ',' << x_vec[k][1] << '\n';
            //}
    ...
    std::cout << "Total time taken: " << duration.count() << " seconds" << endl;
    // Save ll_1 to a file
